...
std::shared_ptr<filter> prepare_filter(const std::string& string_repr) {
    ...
    if (tokens[6] == "any") {
        src_ip_start = 0;
        src_ip_end = std::numeric_limits<uint32_t>::max();
        src_port_start = 0;
        src_port_end = std::numeric_limits<uint32_t>::max();
        ...
        dst_ip_start = 0;
        dst_ip_end = std::numeric_limits<uint32_t>::max();
        dst_port_start = 0;
        dst_port_end = std::numeric_limits<uint32_t>::max();
    } else {
        auto ip_range = get_ip_range(tokens[6]);
        src_ip_start = ip_range.first;
        src_ip_end = ip_range.second;
